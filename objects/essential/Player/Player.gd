...
func player_control_falling(direction: float, delta: float):
	...
	if velocity.y > 0 && velocity.y < 4 * 60:
		velocity.x -= floor(velocity.x / 0.125) / 256.0 * (delta * 60);
...
func player_react_to_collision(prev_floor, prev_velocity, delta):
	...
	if !hit_floor:
		...
		if !prev_floor && rolling && Input.is_action_pressed("player_down") && abs(velocity.length()) >= MIN_ROLL_SPEED:
			roll_sound.play();
...
func player_sprites(direction):
	...
	if sprite.animation == "walk" || sprite.animation == "run":
		sprite.speed_scale = 1.0 / max(0.016, 8 - abs(ground_speed) / 60.0);
	elif sprite.animation == "spin":
		sprite.speed_scale = 1.0 / max(0.016, 4 - abs(ground_speed) / 60.0);
	elif sprite.animation == "push":
		sprite.speed_scale = 1.0 / max(0.016, 8 - abs(ground_speed) / 60.0 * 4);
	else:
...
func update_layer():
	collision_layer = terrain_layer | Global.LAYER_PLAYER;
